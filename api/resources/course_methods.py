...
def create_course_logic(teacher, request):
    try:
        new_course = teacherApplication.create_course(
            teacher,
            request.POST['name'])
        return create_course_success_json_model(new_course.id)
    except Exception as error:
        logger.warn("problem creating course! :" + str(error))
        return create_course_failure_json_model(
            'failed to create the course, sorry. This is probably a db error.')
...
def delete_course_logic(teacher, request):
    try:
        ...
        return delete_course_failure_json_model(
            'failed to delete the course, sorry. This is probably a db error.')
...
def update_course_logic(teacher, request):
    try:
        ...
        new_course = teacherApplication.update_course(
            teacher,
            request.PUT['course_id'],
            request.PUT['name'])
        return update_course_success_json_model()
    except Exception as error:
        logger.warn("problem updating course! :" + str(error))
        return update_course_failure_json_model(
            'failed to update the course, sorry. This is probably a db error.')
...
def get_course_logic(teacher, request):
    try:
        ...
        return get_course_failure_json_model(
            'failed to get the course, sorry. This is probably a db error.')
